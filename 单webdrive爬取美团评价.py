...
def soup_meituan_comment(html, sql):
    ...
    for item in items:
        try:
            ...
            comment_detail = ''
            if len(item.select('.user-comment>span'))>0:
            ...
            # 新增商家回复：
            comment_back = ''
            comment_back_list = item.select('.merchant-reply')
            if len(comment_back_list) > 0:
                print("有商家回复：")
                print(comment_back_list[0].text)
                comment_back = comment_back_list[0].text

            comment_list.append((shop_id, shop_name, usr_image, usr_name, usr_level, deal_name, comment_time,
                                 comment_detail_link, comment_score, comment_detail, comment_photo, comment_op,
                                 comment_reply, comment_back, creat_time))
    ...
    if len(comment_list) > 0:
        sql.insert_comment_info(comment_list)
...
class meituan_sql():
    def __init__(self):
        ...
        '''
        # 1、连接数据库
        self.conn = connect(host='localhost', port=3306, user='root', password='wangying', charset='utf8mb4')
        # 获得Cursor对象
    ...
    def creat_comment_list_db(self):
        ...
        self.cs1.execute('''CREATE TABLE  if not EXISTS meituan_comment_self (
          ...
          `comment_back` varchar(1000) DEFAULT NULL,
